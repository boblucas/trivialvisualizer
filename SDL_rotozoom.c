...
int zoomSurfaceRGBA(SDL_Surface *src, SDL_Surface *dst, int smooth) {
	...
	if (smooth) {
		...
		sx = (int)(1048576.0 * (float)(src->w - 1) / (float)dst->w);
		sy = (int)(1048576.0 * (float)(src->h - 1) / (float)dst->h);
	} else {
		sx = (int)(1048576.0 * (float)src->w / (float)dst->w);
		sy = (int)(1048576.0 * (float)src->h / (float)dst->h);
	}
	...
	if (smooth) {
		...
		for (y = 0; y < dst->h; y++) {
			...
			for (x = 0; x < dst->w; x++) {
				...
				ex = (*csax & 0xfffff);
				ey = (*csay & 0xfffff);
				t1 = ((((c01->r - c00->r) * ex) >> 20) + c00->r) & 0xff;
				t2 = ((((c11->r - c10->r) * ex) >> 20) + c10->r) & 0xff;
				dp->r = (((t2 - t1) * ey) >> 20) + t1;
				t1 = ((((c01->g - c00->g) * ex) >> 20) + c00->g) & 0xff;
				t2 = ((((c11->g - c10->g) * ex) >> 20) + c10->g) & 0xff;
				dp->g = (((t2 - t1) * ey) >> 20) + t1;
				t1 = ((((c01->b - c00->b) * ex) >> 20) + c00->b) & 0xff;
				t2 = ((((c11->b - c10->b) * ex) >> 20) + c10->b) & 0xff;
				dp->b = (((t2 - t1) * ey) >> 20) + t1;
			...
			csay++;
			csp = (tColorRGBA *)((Uint8 *)csp + (*csay >> 20) * src->pitch);
...
void transformSurfaceRGBA(SDL_Surface *src, SDL_Surface *dst, int cx, int cy,
...
SDL_Surface *rotozoomSurface(SDL_Surface *src, double angle, double zoom,
	...
	if ((is32bit) || (src->format->BitsPerPixel == 8)) {
		...
		rz_src = SDL_CreateRGBSurface(SDL_SWSURFACE, src->w, src->h, 32, 0x000000ff,
	...
	if (fabs(angle) > VALUE_LIMIT) {
		...
		if (is32bit) {
			...
			rz_dst = SDL_CreateRGBSurface(
					SDL_SWSURFACE, dstwidth, dstheight, 32, rz_src->format->Rmask,
